...
{
    [self sharedInstance].userKey = userKey;
...
+(void)userLoginCheckResponseOverdueWithUserKey:(NSString *)userKey
{
    ...
    if ([users isKindOfClass:[NSDictionary class]]) {
        ...
        for (userKeyStr in usersDic) {
            id userLastTime = users[userKeyStr];
            if ([userLastTime isKindOfClass:[NSDate class]]) {
                ...
                if (days>JYNetworkResponseCacheOverdueTime) {
                    YYCache *userInfoCache = [YYCache cacheWithName:userKeyStr];
    ...
    userKeyStr = [[JYCache sharedInstance] getCacheUserCacheKey:[self sharedInstance].userKey];
    usersDic[userKeyStr] = currentDate;
...
+(NSString*)getCacheKeyWith:(NSString*)url parameters:(NSDictionary*)parameters{
    ...
    [paramtersDic setObject:[self sharedInstance].userKey forKey:[self sharedInstance].userKey];
...
-(YYCache *)cache
{
    NSString * cacheUserInfoName = [self getCacheUserCacheKey:self.userKey];
    if (!_cache) {
